...
export const UI_STRINGS = (function getLocalization() {
   return {
      ...
      demo_frame_generation: {en: "Frame generation", ru: "Генерация кадров", },
      demo_head_avatar: {en: "Head avatar animation", ru: "Анимированный аватар головы",  },
      demo_full_body_avatar: {en: "Full-body avatar animation",  ru: "Анимированный аватар тела человека",  },
      ...
      demo_procedural_generation: {en: "Procedural mesh generation",  ru: "Процедурная генерация меша",  },
      ...
      huawei_hooks: { en: "Experimented to speed up rendering of \"Genshin Impact\" game, via hooks of OpenGL commands", ru: "Экспериментировал с ускорением рендеринга в Genshin Impact, через хуки OpenGL команд",  },
      huawei_unity_ff_sdk: { en: "Integrated an in-house frame prediction SDK as a Unity plugin (URP pipeline)", ru: "Интегрировал in-house SDK предсказания кадров в Unity плагин для URP пайплайна",  },
      huawei_ohos: { en: "Overall, assisted to develop the ecosystem of the mobile operating system OpenHarmony", ru: "Способствовал развитию графики новой операционной системы OpenHarmony",  },
      samsung_job_title: { en: "Neural Networks R&D", ru: "R&D нейронных сетей", kr: "신경망 연구 개발" },
      samsung_ar_avatars: { en: "Solely created a techdemo to render realistic human avatars, with:", ru: "Один разработал приложение с реалистичными человеческими аватарами",  },
      samsung_rendering_nn: { en: "rendering by neural networks, running 60 FPS in resolution 512x512px on Qualcomm NPU", ru: "рендеринг через нейронную сеть, в 60 FPS и разрешении 512х512 пикселей на Qualcomm NPU",  },
      samsung_animation_system: { en: "my animation system and mesh skinning", ru: "реализовал систему анимации и скининга",  },
      samsung_arcore: { en: "augmented reality via ARCore", ru: "встроил дополненную реальность через ARCore",  },
      samsung_unity: { en: "Ported the techdemo as a Unity AR application", ru: "Портировал приложение в Unity",  },
      samsung_wacv: { en: "Researched and published a paper on neural networks, at WACV 2024 conference: ", ru: "Исследовал и опубликовал научную статью на WACV 2024: ",  },
      samsung_kpi: { en: "In a month completed a crucial yearly KPI of another team", ru: "На месяц включился в другую команду и закрыл для них важный годовой KPI",  },
      ...
      wacv2024_abstract: { en: "The paper improves \"Deferred Neural Rendering\" approach, reducing overfitting to inconsistent training data, by learning offsets to neural texture coordinates for each training image, then discarding them to preserve real-time inference on mobile hardware", ru: "Статья улучшает подход Deferred Neural Rendering, уменьшая оверфиттинг к неконсистентным данным, через оптимизацию warping сетки для нейронной текстуры под каждый кадр в датасете. После обучения warping отключается, для сохранения real-time скорости рендеринга на мобильных устройствах",  },
      wacv2024_author: { en: "I'm the second author of the paper", ru: "Я второй автор статьи",  },
      wacv2024_morphing: { en: "Researched the \"morphing\" idea of the paper", ru: "Исследовал и реализовал warping идею статьи",  },
      wacv2024_demo: { en: "Developed the mobile phone demo that computes avatar images on mobile GPU and Qualcomm NPU in 30-60 FPS", ru: "Разработал технодемку, считающую нейронную сеть прямо на мобильном GPU и Qualcomm NPU",  },
      wacv2024_text: { en: "Prepared a big part of the paper's text and all illustrations", ru: "Подготовил значительную часть текста статьи и все иллюстрации",  },
      ...
      webcv_scratch: { en: "Everything is designed and programmed from scratch", ru: "Все спроектировано и запрограммированно с нуля",  },
      webcv_demos: { en: "Graphics demos:", ru: "Графические демки",  },
      webcv_made_with: { en: "All made with: ", ru: "Реализованы на: ",  },
      webcv_loading: { en: "My implementation of non-blocking demo loading via ", ru: "Сделал неблокирующую загрузку каждой демки через ",  },
      webcv_web: { en: "Web UI:", ru: "Веб UI: ",  },
      webcv_plain: { en: "Plain", ru: "Только",  },
      webcv_vanjs: { en: "and tiny library VanJS for reactive UI", ru: "и маленькая библиотека VanJS для реактивного UI",  },
      webcv_wheel: { en: "The navigation over CV chapters supports mouse wheel scrolling, with transition animations in plain CSS", ru: "Перемещение по главам резюме с помощью колеса мыши, анимации на простом CSS",  },
      webcv_easy: { en: "Easy deployment, no complexity of NodeJS, no webpack", ru: "Простой деплой, без NodeJS или WebPack",  },
      webcv_deploy: { en: "Deployed on my web-server (lighttpd) via GitHub CI/CD", ru: "Хостится на моем личном сервере lighttpd, деплой через GitHub CI/CD",  },
      treesruler_tool: {en: "The tool helps to", ru: "Позволяет",  },
      treesruler_collect: {en: "collect measurements of trees from photos", ru: "замерять деревья по фото",  },
      treesruler_audience: {en: "I designed it for forest scientists. It allows to:", ru: "Я разработал его для ученых лесотехников. Можно:",  },
      treesruler_adjust: {en: "Freely adjust measurements by mouse, snap them to a grid, or set precisely in UI", ru: "Свободно размещать линии, выравнивать по сетке, или точно настраивать в UI",  },
      treesruler_yaml: {en: "Export or import the measurements from a local YAML file", ru: "Делать экспорт/импорт в YAML файлы",  },
      treesruler_grouping: {en: "Add, delete, duplicate, group measurements", ru: "Клонировать, удалять, группировать замеры",  },
      treesruler_settings: {en: "Adjusting visualization and localization", ru: "Конфигурировать стиль отрисовки",  },
      treesruler_libs: {en: "Made with: ", ru: "Сделал с помощью: ",  },
      willreason_page: {en: "Project page", ru: "Страница проекта", kr: "프로젝트 페이지" },
      willreason_videos: {en: "Demo videos", ru: "Демо ролики",  },
      willreason_strategy: {en: "(4X strategy game, unreleased)", ru: "4X стратегическая игра, не выпущена",  },
      willreason_volunteer: {en: "I volunteered during inception of the project:", ru: "Волонтерствовал с самого начала проекта",  },
      willreason_pathfinding: {en: "Implemented pathfinding prototype (A* algorithm)", ru: "Реализовал прототип поиска путей на гексогональной карте (А*)",  },
      willreason_ui: {en: "Developed parts of HUD and game logic of units", ru: "Разрабатывал части UI и игровую логику юнитов",  },
      willreason_game_design: {en: "Engaged in the game design process", ru: "Обсуждал детали гейм-дизайна",  },
      willreason_libs: {en: "Made with: ", ru: "Сделано на: ",  },
      master_title: {en: "MSc of Information Science", ru: "Магистр: Информационные науки и технологии",  },
      ...
      master_research: {en: "Researched augmented reality based on neural networks, supervised by renowned ", ru: "Исследовал дополненную реальность с рендерингом нейронными сетями, под руководством ",  },
      master_victor: {en: "Dr. Victor Lempitsky", ru: "Виктора Лемпитского", kr: "비크토르 렘비트스키" },
      master_thesis: {en: "Defended the thesis on real-time rendering via neural networks on mobile hardware", ru: "Дипломная работа про рендеринг нейронными сетями в реальном времени на мобильных устройствах",  },
      master_courses: {en: "I've taken courses on:", ru: "Прослушал курсы про",  },
      master_deeplearning: {en: "Machine Learning / Deep Learning", ru: "Машинное / Глубинное обучение",  },
      master_3dcv: {en: "3D Computer Vision", ru: "3D компьютерное зрение",  },
      master_parallel: {en: "Parallel computing", ru: "Параллельные вычисления",  },
      master_unity: {en: "Unity game engine", ru: "Игровой движок Unity",  },
      bachelor_title: {en: "BSc of Computer Science", ru: "Бакалавр: Математическое обеспечение информационных систем",  },
      ...
      bachelor_austria: {en: "Winner of a project marathon jointly with TU Graz (Austria)", ru: "Победитель проектного марафона совместно с TU Graz (Австрия)",  },
      bachelor_icpc: {en: "Twice half-finalist of ICPC world olympiad", ru: "Дважды полуфиналист всемирной олимпиады ICPC",  },
      bachelor_badge: {en: "\"Student of the year\" badge", ru: "Именные стипендии и звание \"Студент года\"",  },
      bachelor_thesis: {en: "Defended the thesis on procedural generation of 3D meshes", ru: "Дипломная работа про процедурную генерацию 3D моделей зданий",  },
      bachelor_math: {en: "Math (all core fields)", ru: "Основные разделы математики",  },
      bachelor_architecture: {en: "Computer Architecture", ru: "Архитектуру компьютера",  },
      bachelor_oracle: {en: "Oracle Database administration", ru: "Администрирование Oracle баз данных",  },
      // a: {en: "", ru: "",  },
   };
})()
